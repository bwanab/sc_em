defmodule MidiPlayer do
  ...
  @spec message_worker(
          [{:program_change | :tempo | :noteon | :noteoff | :cc_event | :end_of_track, map}],
          map
        ) :: any
  def message_worker([%{type: type, val: _val} | _rest], state) when type == :end_of_track do
    state
  end
  ...
  def message_worker([%{type: type, val: val} | rest], state) do
    ...
    s =
      case type do
        ...
        :tempo ->
          tickdiv = val.val / 1_000_000 / state.tpqn
  ...
  def wait(delta, state) do
    if delta > 0 do
      ...
      Logger.info(
        "delta = #{delta} state.tickdiv = #{state.tickdiv} ms_to_sleep = #{ms_to_sleep}"
      )
  ...
  def test_notes(n, delta) do
    ...
    |> message_worker(initial_state(midi))
