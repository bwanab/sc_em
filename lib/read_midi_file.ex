defmodule ReadMidiFile do
  ...
  def read_tracks(_d, _n, num) when num == 0 do
    []
  end
  ...
  def read_messages(d, n, last_m_type, n_offset) do
    ...
    try do
      ...
      e in CondClauseError ->
        Logger.warning("m_type = #{Integer.to_string(m_type, 16)} n = #{n}")
        e
    end
  end
  ...
  def sysex_message(delta, d, n) do
    ...
    {%MidiMessage{
       type: :sysex_event,
       val: %{:delta => delta, :sysex => Enum.slice(d, n1, length - 2)}
     }, n1 + length}
  end
  ...
  def meta_message(delta, d, n) do
    ...
    try do
      ...
      e in CaseClauseError ->
        Logger.warning("meta_type = #{meta_type} n = #{n}")
        e
    end
  end
  ...
  def get_string_val(event, delta, d, n, length) do
    val =
      if length == 0 do
        ...
        List.to_string(Enum.slice(d, n, length))
      end
  ...
  def smpte_offset(delta, d, n) do
    ...
    {%MidiMessage{
       type: :smpte_offset,
       val: %{:hr => hr, :mn => mn, :se => se, :fr => fr, :ff => ff, :delta => delta}
     }, n5}
  end
  ...
  def note(channel, delta, d, n) do
    ...
    if vel == 0 do
      {%MidiMessage{
         type: :noteoff,
         val: %{:channel => channel, :delta => delta, :note => note, :vel => vel}
       }, n2}
    else
      {%MidiMessage{
         type: :noteon,
  ...
  def polyphonic_pressure(channel, delta, d, n) do
    ...
    {%MidiMessage{
       ...
       val: %{:channel => channel, :delta => delta, :note => note, :val => val}
     }, n2}
  end
  ...
  def control_change(channel, delta, d, n) do
    ...
    {%MidiMessage{
       type: :cc_event,
       val: %{:channel => channel, :delta => delta, :cc => cc, :val => val}
     }, n2}
  end
  ...
  def pitch_wheel(channel, delta, d, n) do
    ...
    {%MidiMessage{
       ...
       val: %{:channel => channel, :delta => delta, :lsb => lsb, :msb => msb}
     }, n2}
  end
  ...
  def aftertouch(channel, delta, d, n) do
    ...
    {%MidiMessage{
       ...
       val: %{:channel => channel, :delta => delta, :pressure => pressure}
     }, n1}
  end
  ...
  def program_change(channel, delta, d, n) do
    ...
    {%MidiMessage{
       type: :program_change,
       val: %{:channel => channel, :delta => delta, :program => program + 1}
     }, n1}
  end
end
